...
useEffect(() => {
  ...
  const fetchCustomerStats = async () => {
    ...
    try {
      const res = await fetch(`http://localhost:3030/api/stats/customers?manifest=${encodeURIComponent(selectedManifest)}`);
...
useEffect(() => {
  const loadManifests = async () => {
    try {
      const res = await fetch('http://localhost:3030/api/manifests/scan-stats');
      const data = await res.json();
      if (data.success) {
...
const loadManifestDetails = async (manifestNumber) => {
  ...
  try {
    const res = await fetch(`http://localhost:3030/api/manifests/${encodeURIComponent(manifestNumber)}`);
    const data = await res.json();
    
    if (data.success) {
      // Also fetch the scan activity for this manifest
      const activityRes = await fetch(`http://localhost:3030/api/manifests/${encodeURIComponent(manifestNumber)}/scans`);
...
const fetchCustomerStats = async (manifestNumber = null) => {
  ...
  try {
    if (manifestNumber === 'UNMANIFESTED') {
      ...
      const res = await fetch(`http://localhost:3030/api/manifests/UNMANIFESTED`);
      ...
      const res = await fetch(`http://localhost:3030/api/stats/customers?manifest=${encodeURIComponent(manifestToFetch)}`);
...
const updateGRDMSOrder = async (parcelData) => {
  try {
    ...
    const response = await fetch('http://localhost:3030/api/orders/update', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
